...
Phase 4A — Customer & Supplier Domain Design is current, documented, and awaiting manual approval. The proposed design keeps Customer and Supplier as separate Company-owned master-data entities; defines fields, maximum lengths, normalization, deliberately non-unique names/contact details, UUID/timestamp conventions, tenant-aware future relationships, restrictive deletion, archive/reactivation, OWNER/ADMIN management, STAFF read-only access, strict future PATCH behavior, and tenant-local not-found behavior; and requires future transactional documents to snapshot mutable partner details where their own design needs historical accuracy. Phase 4A is documentation only. No Customer/Supplier Prisma model, migration, API, frontend feature, Purchasing, Sales, or AI implementation has been added. Phase 4B and Phase 5 have not started.
