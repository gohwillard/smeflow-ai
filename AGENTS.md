...
Phase 3C — Category & Product Backend is implemented and Codex-verified, awaiting user/ChatGPT manual API verification before approval. The authenticated Category and Product APIs use strict Zod contracts, database-validated Company scope, OWNER/ADMIN writes, STAFF reads, safe archive behavior, active same-Company Category assignment, fixed-scale exact decimal strings, and read-only `quantityOnHand`. All 136 backend tests pass. No schema, migration, frontend, InventoryMovement API, stock-adjustment service, search, low-stock, or Phase 3D+ work was added. Do not start Phase 3D without explicit user instruction after Phase 3C manual review.
