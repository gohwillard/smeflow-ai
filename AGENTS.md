...
Phase 4C — Customer & Supplier Backend is current, implemented, and Codex-verified, awaiting manual API/database approval. Authenticated Customer and Supplier list, detail, create, partial-update, idempotent archive, and explicit reactivation APIs use strict Zod contracts, server-side normalization, safe response selections, database-validated `req.auth.companyId` scope, tenant-local not-found behavior, OWNER/ADMIN writes, and STAFF reads. Duplicate business data remains allowed. All 316 backend tests across 10 files pass, including 109 focused Phase 4C tests, together with backend typecheck/build, Prisma validation/generation/migration status, and repository hygiene checks. No dependency, schema, migration, frontend, search, pagination, Purchasing, Sales, Dashboard, or AI change was added. Phase 4D and Phase 5 have not started.
