...
Phase 4 — Customers and Suppliers is officially complete after final manual acceptance.
...
Phase 4G — Phase 4 Final Verification is complete after final manual approval. The complete Customer and Supplier subsystem passed domain and scope review; live PostgreSQL column, constraint, index, foreign-key, and migration inspection; migration-to-live and schema-to-live no-drift comparisons; strict request/query validation; OWNER/ADMIN/STAFF authorization; Company isolation; archive/reactivation; search/filter AND semantics; frontend integration; accessibility and responsive review; Phase 3 and authentication regression; and secret/dependency/generated-file hygiene. Four focused backend cases explicitly prove archived-filter tenant isolation and `pageSize` rejection for both modules. All 411 backend tests across 12 files and all 171 frontend tests across 8 files pass. Backend typecheck/build, frontend ESLint/build, Prisma validate/generate/migrate status, exactly three migrations, and `git diff --check` pass. Final manual verification approved representative CRUD/lifecycle, search/filter, role, Company-isolation, responsive, custom Status dropdown, transaction-history foundation, database/migration, and Phase 3 regression behavior.

There is currently no active development milestone. Phase 5 — Purchasing has not started and must not begin without explicit user authorization. Preserve all approved Phase 4 Company-isolation, authorization, lifecycle, search/filter, database-integrity, frontend, and future-history invariants when later phases are designed.
